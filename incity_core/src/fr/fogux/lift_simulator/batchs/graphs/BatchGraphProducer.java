...
public class BatchGraphProducer extends SimuBatch
{
	...
	public BatchGraphProducer(File dossierDuBatch, DataTagCompound batchConfig) throws IOException 
	{
		super(dossierDuBatch,batchConfig.getLong(TagNames.seed));
		echantillonage = batchConfig.getInt(TagNames.echantillonage);
		File configFile = GestFichiers.getUniqueFile(dossierDuBatch, NomsFichiers.graph_batch_config);
		FichierGraphBatchConfig graphBatchConfigFile = GestFichiers.getGraphBatchConfig(configFile);
	...
	private List<StringProvider> getAbscisse()
	{
		...
		List<List<? extends StringProvider>> lists = new ArrayList<>();
	...
	{
		File graphFile = GestFichiers.createGraphBatchResultFile(dossier);
	...
	public synchronized void registerCompleted(PointGraph point)
	{
		queue.add(point);
		tryVidageQueue();
	}
	...
	public void runBatch() 
	{
		...
		PartitionGenInstantiator pgenInstantiator = new PartitionGenInstantiator(strPartition,new DataTagCompound(strImmeuble.getString()));
		
		List<StringProvider> abscisses = getAbscisse();
		do 
		{
			DataTagCompound compoundImmeuble = new DataTagCompound(strImmeuble.getString());
			do
			{
				PartitionGenerator pGen = pgenInstantiator.getPartitionGen();
				do
				{
					
					DataTagCompound compoundSimu = new DataTagCompound(strSimu.getString());
					compoundSimu.mergeWith(compoundImmeuble);
					ConfigSimu cSimu = new ConfigSimu(compoundSimu);
					
					
					SimuTaskReceiver<?> point = new PointGraph(this, echantillonage, identifiantPoint,"(" + StringProvider.getFullString(abscisses) + ")");
					identifiantPoint ++;
