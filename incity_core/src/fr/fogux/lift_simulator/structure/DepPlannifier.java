...
public class DepPlannifier implements Compoundable
{
    ...
    public DepPlannifier(final ConfigSimu c, final AscState EI)
    {
        this.c = c;
        this.EI = EI;
        EF = EI;
    ...
    public DepPlannifier(final DepPlannifier shadowed)
    {
        c = shadowed.c;
        EI = shadowed.EI;
        EF = shadowed.EF;
    ...
    public void initiateMovement(final long t0, final float xf, final DepPlannifier upper, final DepPlannifier lower)
    {
        EI = getFullState(t0);
        virgules.clear();
     ...
     */
    private AscState initiateMovementPotentialBoundTo(final DepPlannifier fixedPlannifier, final float xf, final AscSoftDep directDep)
    {
        ...
        if( fixedPlannifier == null
            ...
            ||
            (fixedPlannifier.isSimpleVirgule() && c.getAscenseurSpeed()*(fixedPlannifier.getEtatInitial().t+c.getDeltaT() - EI.t) <= Math.abs(fixedPlannifier.getEtatInitial().x - EI.x))
            )
        {
        ...
        else
        {
            ...
            if(v1CollisionIntoV2.apply(c, AscDeplacementFunc.getMaxXPossible(fixedPlannifier.EF.t, c, EI, aPositif), fixedPlannifier.EF.x))
            {
                ...
                {
                    final AscVirgule av = fixedPlannifier.virgules.get(i);
                ...
                final AscVirgule lienPotentiel = getPotentielLien(fixedPlannifier.virgules.get(fixedPlannifier.virgules.size() - 1),xOffset);
    ...
    private AscState remplirVirgulesLien(final AscVirgule lien, final DepPlannifier ascBounded, int index, final float xf, final DoublePredicate collisionV1intoV2, final float xOffset)
    {
        virgules.add(lien);
