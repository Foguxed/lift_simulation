...
public class EtageSimu extends Etage
{
    ...
    public void ouvrir(AscenseurSimu asc)
    {
        if(ouvertureEnCours)
        {
            throw new SimulateurAcceptableException("Ouverture des portes impossible car elles sont deja en cours d'ouverture sur l'asc " + asc.getId());
        }
    ...
    public void fermer(AscenseurSimu asc)
    {
        if(transferEnCours)
        {
            throw new SimulateurAcceptableException("Fermeture des portes impossible si les personnes ne sont pas encore toutes entrees. etage" + num);
        }
