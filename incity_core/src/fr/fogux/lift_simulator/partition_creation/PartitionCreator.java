...
public class PartitionCreator
{
     ...
     * notre paquebot https://www.msccroisieres.fr/fr-fr/Decouvrez-MSC/Navires-De-Croisiere/MSC-Opera.aspx
     * a les ponts 5 a 13, tous les etages ont la meme proba d etre selectionnes en depart/arrivee
     * 9 etages avec le 0 compris donc 0 a 8
     */
    protected void generatePaquebotNormal()
    {
        ...
        double[] grps = {0.20d,0.724d,0.0055d,0.0140};
           ...
           int nbPersonnesDeplacees,long duree)
    {
        ...
        double[] grps = {0.85d,0.10d,0.03d,0.02d};
        GroupProfile grpProfile = new GroupProfile(grps);
        ...
        for(int i = 1; i < etageMax +1 ;  i ++)
        {
            ...
            depart.add(new EtageDeDepart(i,(probaOutCome + probaInterEtage)/(double)etageMax,profilEtages,grpProfile));
        }
    ...
    protected List<EtageDestination> getDestinationsProbaEgales(final int etageMin,final int etageMax, double probaTotaleDuGroupe)
    {
