...

{
    ...
    public AscenseurVisu(int id, int personnesMax,float x)
    {
        ...
        drawableObject.addRelativeDrawable(drawableBoutonList, new Vector2(sprite.getWidth()/2 +40,sprite.getHeight()/2));
    ...
    protected void updateGroupsPos()
    {
        ...
        if(listGroupes.size() > nbMaxPremiereLigne)
        {
            ...
            {
                listGroupes.get(i).repositionnerDansAscenseur(this,newSizeMult,
                minX+newEcart/2+newEcart*(i-nbPremiereLigne),30+150*sizeMult);
            }
        }
    }
    ...
    protected void updateBoutonList()
    {
        ...
        {
            drawableBoutonList.changerRelativePosition(obj, new Vector2(0,y));//ajoute automatiquement si l'objet n'est pas dedans
            y += espaceEntreChaque;
        }
