...
public class ImmeubleVisu implements PredictedDrawable
{
    ...
    /*@Override
    public Ascenseur generateAscenseur(int id, int personnesMax)
    {
        final AscenseurVisu asc = new AscenseurVisu(id, personnesMax,lastAscenScreenX);
        lastAscenScreenX += ecartAscenseurs;
        return asc;
    }

    @Override
    public Etage<?> generateEtage(int num)
    {
        final EtageVisu etage = new EtageVisu(num,lastLevelScreenY);
        lastLevelScreenY += 380;
        return etage;
    }*/
    ...
    {
        //System.out.println(" etages "+ etages.length + " niveau min " + niveauMin + " niveau " + niveau);
